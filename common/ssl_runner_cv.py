...
def postTrain_cv():
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(full_ds)):
        ...
        log.info("=" * 90)
        if task == "ssl_posttrain_a1":
            log.info("   Fold  |  Epoch  |    LR     | Train Loss | Val Loss | F1 raw | F1 sel |  AUROC | F1[D/A/S]       | Time")
        elif task == "ssl_posttrain_a2":
            log.info("   Fold  |  Epoch  |    LR     | Train Loss | Val Loss | mean QWK | mean MAE | Time")
        log.info("=" * 90)
        ...
        for epoch in range(1, epochs + 1):
            ...
            if task == "ssl_posttrain_a1":
                ...
                log.info(
                    f"  {fold:3d} | {epoch:3d}/{epochs:3d} | {lr_now:.2e} |   {train_loss:.4f}   |  {val_metrics['loss']:.4f}  | "
        ...
        state = load_checkpoint(run_dirs["checkpoints"] / f"best-{fold}.pt", ssl_model, optimizer=None)
